...
本文档用于规范 `@drift-ui/ui` 的后续版本发布流程。默认采用 `Changesets + GitHub Actions + npm` 自动发布链路。
...
2. 确认 npm 包名可用：`npm view @drift-ui/ui`
...
```bash
npm deprecate @drift-ui/ui@X.Y.Z "This version has critical issues, please upgrade."
```
